...
PROGRAM MF3_Adapter
	...
	IF(COMMAND_ARGUMENT_COUNT() /= 2 .AND. COMMAND_ARGUMENT_COUNT() /= 3) THEN
		PRINT *, "mf3 - Meteoflux V3.x adapter for SonicLib"
		PRINT *
		PRINT *,"Usage:"
		PRINT *
		PRINT *,"  ./mf3 <InputFile> [<CvtFile>] <OutputFile>"
		PRINT *
		...
		PRINT *
		PRINT *,"<ChannelNo> is an integer from 1 to the maximum channel ID (4 in case"
		PRINT *,"            of Meteoflux V3.x / Metek USA1)"
		PRINT *
	...
	IF(lConvertAnalog) THEN
		...
		IF(iRetCode /= 0) THEN
			PRINT *,"mf3:: warning: Conversion file not found, converting only sonic quadruples"
		...
		ELSE
			...
			IF(iNumCvtSpecs <= 0) THEN
				PRINT *,"mf3:: warning: Conversion file is empty, converting only sonic quadruples"
				lConvertAnalog = .FALSE.
			ELSE
		...
		! Check all channels are within proper limits
		DO i = 1, iNumCvtSpecs
			IF(ivChannel(i) < 1 .OR. ivChannel(i) > NUM_COLS-4) THEN
				PRINT *,"mf3:: warning: One channel number in conversion file exceeds limits, converting only sonic quadruples"
				lConvertAnalog = .FALSE.
			END IF
		END DO
...
loop:	DO i = 1, iNumCvtSpecs-1
		...
		END DO loop
		IF(lTwoEquals) THEN
			PRINT *,"mf3:: warning: Two identical channels in conversion file, converting only sonic quadruples"
			lConvertAnalog = .FALSE.
		END IF
		DO i = 1, iNumCvtSpecs
			lQuantityFound = .TRUE.
			...
			IF(.NOT.lQuantityFound) THEN
				PRINT *,"mf3:: warning: Quantity ", TRIM(svVarName(i)), " at channel ", ivChannel(i), "not found"
				lConvertAnalog = .FALSE.
			END IF
		END DO
	...
	IF(iRetCode /= 0) THEN
		PRINT *,"mf3:: error: Input file not opened"
		STOP
	END IF
	iNumData = 0
	DO
		iRetCode = GetRecord(10, iSecond, ivValues)
		IF(iRetCode /= 0) EXIT
		iNumData = iNumData + 1
	END DO
	IF(iNumData < 100) THEN
		PRINT *,"mf3:: error: Insufficient data in input file"
		STOP
	END IF
	ALLOCATE(imRecord(iNumData, NUM_COLS), ivTimeStamp(iNumData), rvTimeStamp(iNumData), STAT=iRetCode)
	IF(iRetCode /= 0) THEN
		PRINT *,"mf3:: error: Impossible to reserve workspace in internal memory"
		STOP
	END IF
	imRecord = -99999
	REWIND(10)
	...
	DO
		...
		iData = iData + 1
		ivTimeStamp(iData)  = iSecond
		imRecord(iData,1)   = ivValues(2)
		imRecord(iData,2)   = ivValues(1)
		imRecord(iData,3:8) = ivValues(3:8)
	...
	IF(iNonEmptyCols < 4) THEN
		PRINT *,"mf3:: error: At least one column in sonic quadruple missing"
		STOP
	END IF
	...
	IF(lConvertAnalog .AND. iNumUsedChannels <= 4) THEN
		PRINT *,"mf3:: warning: None of the analog channels to convert exists in data set"
		lConvertAnalog = .FALSE.
	END IF
	
	! Re-attribute time stamps
	rvTimeStamp = [(3600.*FLOAT(iData)/FLOAT(iNumData),iData=0,iNumData-1)]
	...
	DO iData = 1, iNumData
		DO iChannel = 1, iNumUsedChannels
			...
			END IF
		END DO
		WRITE(sBuffer, "(1x,f7.2,8(',',f9.3))") rvTimeStamp(iData), rvRecord(1:iNumUsedChannels)
		iPosLastChar = LEN_TRIM(sBuffer)
		IF(sBuffer(iPosLastChar:iPosLastChar) == ",") sBuffer(iPosLastChar:iPosLastChar) = " "
		WRITE(10, "(a)") TRIM(sBuffer)
	END DO
	...
	! Leave
	DEALLOCATE(imRecord, ivTimeStamp, rvTimeStamp)
	IF(lConvertAnalog) DEALLOCATE(ivChannel, rvMultiplier, rvOffset, svVarName)
	
CONTAINS

	FUNCTION GetRecord(iLUN, iTimeStamp, ivValues) RESULT(iRetCode)
	
		! Routine arguments
		INTEGER, INTENT(IN)						:: iLUN
		INTEGER, INTENT(OUT)						:: iTimeStamp
		INTEGER(2), DIMENSION(8), INTENT(OUT)	:: ivValues
		INTEGER									:: iRetCode
		...
		READ(iLUN, IOSTAT=iErrCode) cChar, iTimeStamp, ivValues
		IF(iErrCode /= 0 .OR. iTimeStamp < 0) THEN
			iRetCode = 1
			RETURN
		END IF
